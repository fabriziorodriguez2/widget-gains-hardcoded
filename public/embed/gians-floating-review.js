(function () {
  ...
  const STORAGE_KEY_NEXT_ALLOWED_AT = "flikker_review_next_allowed_at";
  const STORAGE_KEY_REVIEW_INDEX = "flikker_review_index";
  ...
  function getNextAllowedAt() {
    try {
      return Number(localStorage.getItem(STORAGE_KEY_NEXT_ALLOWED_AT) || "0");
    } catch (error) {
      return 0;
    }
  }

  function setNextAllowedAt(timestamp) {
    try {
      localStorage.setItem(STORAGE_KEY_NEXT_ALLOWED_AT, String(timestamp));
    } catch (error) {}
  }

  function getSavedIndex() {
    try {
      return Number(localStorage.getItem(STORAGE_KEY_REVIEW_INDEX) || "0");
    } catch (error) {
      return 0;
    }
  }

  function setSavedIndex(value) {
    try {
      localStorage.setItem(STORAGE_KEY_REVIEW_INDEX, String(value));
    } catch (error) {}
  }

  function canShowNow() {
    return getNow() >= getNextAllowedAt();
  }

  function scheduleNextAllowedFromNow() {
    setNextAllowedAt(getNow() + COOLDOWN_MS);
  }
  ...
  function start() {
    index = getSavedIndex() % reviews.length;
